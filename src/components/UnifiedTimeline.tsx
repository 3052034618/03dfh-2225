  ...
  return (
    <div className="card p-6 mb-6">
      ...
      {(selectedAlert || selectedEvent) && (
        <div className="mt-6 p-6 bg-gradient-to-br from-primary-50 to-white rounded-xl border border-primary-100">
          <h4 className="text-sm font-semibold text-primary-700 mb-4 flex items-center gap-2">
            <Phone className="w-4 h-4" />
            客服解释话术（点击即可查看）
          </h4>
          <div className="bg-white rounded-lg p-4 border border-primary-200 text-sm text-neutral-700 space-y-3">
            <p>
            ...
            <p>
              ...
              {selectedAlert
                ? `温度最高达到 ${formatTemperature(selectedAlert.maxTemperature)}，持续了 ${formatDuration(selectedAlert.durationMinutes)}，发生在 ${selectedAlert.location}。`
                : selectedEvent
            ...
            {selectedAlertAction && (
              <p>
                ...
                {selectedAlertAction.action}，处理结果是 {selectedAlertAction.result.toLowerCase()}。
              </p>
            )}
            {selectedEventAction && !selectedAlertAction && (
              <p>
                <span className="font-medium text-success-600">我们当时的处理：</span>
                {selectedEventAction.action}，处理结果是 {selectedEventAction.result.toLowerCase()}。
              </p>
            )}
